 ...
 * 물때 판정.
 *
 * 그날의 만조·간조 시각은 앱이 뜰 때 국립해양조사원 API로 런타임에 받아옵니다
 * (src/lib/stations.ts). 사리/조금 판정 기준선(연중 조차 분포)만 미리 구해
 * stations-index.ts 에 구워뒀어요 — 조석은 천문학적으로 결정되기 때문에
 * 해가 바뀌어도 거의 안 변합니다.
 *
 * 이 파일이 하는 일은 "숫자를 사람 말로 바꾸는" 것뿐이에요.
